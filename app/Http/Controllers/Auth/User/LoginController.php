...
class LoginController extends Controller
{
   ...
   /**
        ...
        try {
            ...
            if($userId!=''){
                ...
                if($this->saveOTP($data)){
                    return view('user.otpLoginForm',['mobile'=>$mobile,'pass'=>$password,'userId'=>$id]);
                }
            
            }else{
                return redirect('user/login')->with(['status'=>'Sorry ! Invalid Url.']);
            }
    ...
    {   
        ...
        try {
            ...
            if($loginDetails!=null){
                ...
                return redirect('/verifyOTP/'.$user_id.'/'.$pass)->with(['status'=>'Invalid OTP!!']);
            }
            


        } catch (DecryptException $e) {
            return redirect('/verifyOTP/'.$user_id.'/'.$pass)->with(['status'=>'Sorry ! Invalid Url, try again']);
        }
    ...
    public function login(Request $request)
    {
        ...
        //Redirect Page For OTP
        if($user AND $user->role_id == 2){
            $id         =  Crypt::encryptString($user['id']);
            $password   =  Crypt::encryptString($request->get('password'));
            return redirect('/verifyOTP/'.$id.'/'.$password)->with(['status'=>'Verify OTP Now.']);
        }
